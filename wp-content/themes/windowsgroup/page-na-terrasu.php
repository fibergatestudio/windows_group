...
<div class="main page-windows page-type-windows">
   ...
   <section class="section-table-price">
      <div class="container">
         <div class="top-post-page">
            <h1 class="title-post">Двери на террасу в Киеве</h1>
            <p class="text-custom-page">Наша компания “Виндовс Груп” предлагает купить на заказ пластиковые двери на террасу в Киеве и области по лучшим ценам.
                В каталогах представлены популярные модели дверных конструкций от известных производителей.</p>
         </div>

        <?php 
            
            global $wpdb;
            $result_params = $wpdb->get_results("SELECT * FROM `{$wpdb -> prefix}window_prices` WHERE `page_name` = 'dvery_na_terrasu' ");
            $price_obj = json_decode($result_params[0]->data);
         
        ?>
         
         <div class="block-windows-type-1">
            <h2 class="title-block-type-windows">Раздвижные металлопластиковые двери на террасу</h2>
            <span class="small-title">Размер 2000 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->razdvizniye_1->min?></span> грн до <span class="max-price"><?=$price_obj->razdvizniye_1->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-open">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->razdvizniye_1 as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-open" role="tabpanel" aria-labelledby="<?=$key_w?>-open">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/1-razdvizhnyye-metalloplastikovyye-dveri-na-terrasu.8d0830da.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->razdvizniye_1->$key_w;
                              $j = 0;
         ...
         <div class="block-windows-type-2">
            <h2 class="title-block-type-windows">Двухстворчатые панорамные металлопластиковые двери на террасу</h2>
            <span class="small-title">Размер 2700 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->razdvizniye_2->min?></span> грн до <span class="max-price"><?=$price_obj->razdvizniye_2->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->razdvizniye_2 as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>" role="tabpanel" aria-labelledby="<?=$key_w?>">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/2-raspashnyye-metalloplastikovyye-dveri-na-terrasu.1a4ea080.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->razdvizniye_2->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-4">
            <h2 class="title-block-type-windows">Металлопластиковые двери на террасу со штульпом</h2>
            <span class="small-title">Размер 1400 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->so_shtulpom->min?></span> грн до <span class="max-price"><?=$price_obj->so_shtulpom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-4">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->so_shtulpom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-4" role="tabpanel" aria-labelledby="<?=$key_w?>-type-4">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/4-metalloplastikovyye-dveri-na-terrasu-so-shtulpom.dd7bb52f.png" class="vertical-doors-2 img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->so_shtulpom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-3">
            <h2 class="title-block-type-windows">Металлопластиковые двери на террасу гармошка</h2>
            <span class="small-title">Размер 2700 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->garmoshka->min?></span> грн до <span class="max-price"><?=$price_obj->garmoshka->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-3">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->garmoshka as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>
                  ...
                  <div class="table-tab">
                     ...
                     <table class="table-windows table-price-windows">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->garmoshka->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
   ...
   <section class="section-post section-post-windows">
      <div class="container">
         <h2 class="title-section">Преимущества и недостатки дверей на террасу</h2>
         <p>При обустройстве загородного дома хочется создать уютное место для отдыха. Установка качественных дверных конструкций сделает помещение стильным и защищенным от взлома.</p>
         <p>Двери для террасы характеризуются преимуществами:</p>
         <ul>
            <li>· безопасностью;</li>
            <li>· теплоизоляцией;</li>
            <li>· возможностью проветривания и защиты от перегрева помещения;</li>
            <li>· удобством эксплуатации;</li>
            <li>· долговечностью конструкции;</li>
            <li>· естественной освещенностью комнаты.</li>
         </ul>
         <p>Оптимальный вариант – распашные и раздвижные модели. Выход на террасу может снабжаться классической дверью, конструкцией с частичным или цельным остеклением.</p>
         <h2 class="title-post">Особенности</h2>
         <p>Каждый тип открывания имеет особенности. Распашные конструкции являются стандартным и экономичным решением. Двухстворчатые типы используются для визуального расширения пространства. Распашные модели отличаются легкой установкой. При необходимости дверные конструкции оснащаются автоматическим доводчиком.</p>
         <p>Раздвижные двери для выхода на террасу помогают сэкономить площадь помещения. Скользящие конструкции защищены от непроизвольного открывания, передвигаются по роликовому механизму. При необходимости нижняя направляющая углубляется в пол. При открывании это незаметно разделяет одно помещение от другого.</p>
         <p>Раздвижные модели выдерживают большие нагрузки, поэтому полотно может полностью остекляться. При эксплуатации исключается вероятность случайного травматизма при захлопывании.</p>
         <p>Распространенным видом двери считается портальная гармошка. Створки перемещаются словно гармошка, широко открывая проем. В порталах отсутствуют импосты и глухие секции. Доводчик обеспечивает мягкое открывание и закрывание конструкции.</p>
         <p>Типы актуальных моделей дверей на террасу на фото показаны на сайте.</p>
         <h2 class="title-post">Кому подойдут</h2>
         <p>Конструкции распашного типа являются выгодным вариантом классической дверной модели. Ширина распашного полотна ограничена по причине не выдерживания петлями большого веса. Для широкого проема на две створки допускается монтаж дверей с разной шириной. При этом узкая дверь устанавливается без открывания.</p>
         <p>Раздвижные модели функционируют по принципу шкафа-купе. Сдвигание створок осуществляется в глухую часть остекления или фальш-стену. Конструкции смотрятся необычно и стильно. Подходят для монтажа между закрытой террасой и гостиной.</p>
         <p>Портальные двери на террасу применяются в габаритных проемах. Раздвижные портальные конструкции соединяют дом с зимним садом, бассейном и оранжереей. Открывание порталов возможно внутрь и наружу.</p>
         <p>Модель «гармошка» устанавливается при входе на террасу с ограниченным пространством. Полотна передвигаются по направляющим снизу и сверху без отодвигания, а створки складываются по профилю рамы. Сдвижные части могут комбинироваться с глухим остеклением.</p>
         <h2 class="title-post">Как выбрать профиль</h2>
         <p>При выборе профиля учитываются моменты:</p>
         <ol>
            <li>Если дверная конструкция предназначается для защиты от осадков и ветра летом, тогда достаточно использовать легкий алюминиевый профиль. Температура внутри помещения одинаковая, как снаружи.</li>
            <li>Для входной пластиковой двери на террасу устанавливаются модели из металлопластика с армирующим каркасом. Армирование металла обеспечивает высокую тепло- и звукоизоляцию.</li>
            <li>Размеры дверей на террасу подбираются с учетом размера и особенностей проема. Наши специалисты готовы грамотно осуществить замеры и проконсультировать по продукции.</li>
            <li>Покупка комплектных дверных систем с коробкой, уплотнителем, крепежом и фурнитурой гарантирует надежную и длительную эксплуатацию двери.</li>
         </ol>
         <p>При покупке следует выбирать конструкции с двойным роликовым механизмом и двумя направляющими рельсами. Модели отличаются плавным и перемещением дверного полотна, защитой от заклинивания и перекашивания.</p>
         <h2 class="title-post">Цены на пластиковые двери на террасу</h2>
         <p>Цена дверей на террасу складывается с учетом типа, эстетических и функциональных особенностей конструкции. Узнать ориентировочную стоимость изделий в Украине можно на сайте компании «Windows Group» или, позвонив по телефону.</p>
      </div>
   </section>
      ...
      <div class="container">
         ...
         <ul class="list-other-type-windows">
            <li class="item-other-type-windows">
               <div class="block-img-other doors"><img src="<?php bloginfo('template_url') ?>/images/img-unlinking-doors.c4ae7130.webp" class="not-all-img img-fluid" alt="photo"></div>
               <a href="<?=get_bloginfo('home')?>/plastikovye-dveri/" class="title-footer">Металопластиковые двери</a>
            </li>
            <li class="item-other-type-windows">
