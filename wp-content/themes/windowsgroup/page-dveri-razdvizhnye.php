<?php
/* Template Name: PageDveriRazdvizhnye */
get_header();
?>
...
<div class="main page-windows page-type-windows">
   ...
   <section class="section-table-price">
      <div class="container">
         <div class="top-post-page">
             <h1 class="title-post">Раздвижные металлопластиковые двери</h1>
             <p class="text-custom-page">Раздвижные двери – это решение, которое может быть принято на любом этапе строительства или ремонта жилья.
                 Замена традиционных межкомнатных дверей раздвижными моделями позволит сэкономить необходимое пространство.
                 Компания «Виндовс Груп» предлагает купить раздвижные двери в Киеве и области по оптимальной стоимости от производителя.</p>
         </div>

         <?php 
            
            global $wpdb;
            $result_params = $wpdb->get_results("SELECT * FROM `{$wpdb -> prefix}window_prices` WHERE `page_name` = 'dvery_razdvizniye' ");
            $price_obj = json_decode($result_params[0]->data);
         
         ?>
         
         <div class="block-windows-type-1">
            <h2 class="title-block-type-windows">Раздвижные металлопластиковые двери</h2>
            <span class="small-title">Размер 1000 x 2100 мм, Цена от <span class="min-price"><?=$price_obj->razdvizniye->min?></span> грн до <span class="max-price"><?=$price_obj->razdvizniye->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-open">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->razdvizniye as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-open" role="tabpanel" aria-labelledby="<?=$key_w?>-open">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/1-razdvizhnyye-metalloplastikovyye-dveri-na-terrasu.8d0830da.png" class="mt-lg-4 img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->razdvizniye->$key_w;
                              $j = 0;
         ...
         <div class="block-windows-type-2">
            <h2 class="title-block-type-windows">Раздвижные металлопластиковые двери с панорамными окнами по бокам</h2>
            <span class="small-title">Размер 3000 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->razdvizniye_s_oknamy->min?></span> грн до <span class="max-price"><?=$price_obj->razdvizniye_s_oknamy->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->razdvizniye_s_oknamy as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>" role="tabpanel" aria-labelledby="<?=$key_w?>">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/2-razdvizhnyye-metalloplastikovyye-dveri-s-panoramnymi-oknami-po-bokam.92c2eed7.png" class="mt-lg-4 img-type-windows-3 img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->razdvizniye_s_oknamy->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
        ...
        <!--  <div class="block-windows-type-3">
            ...
            <div class="tab-content" id="myTabContent-type-3">
               <div class="tab-pane fade show active" id="wds-type-3" role="tabpanel" aria-labelledby="wds-type-3">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/3-metalloplastikovyye-dveri-na-terrasu-garmoshka.326f93d6.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
   ...
   <section class="section-post section-post-windows">
      <div class="container">
         <h2 class="title-section">Разновидности раздвижных дверей</h2>
         <p>Существуют различные раздвижные двери, цена на которые зависит от вида товара:</p>
         <ol>
            <li>Параллельно-раздвижная конструкция. Движение створки осуществляется по направляющим в параллельной плоскости проема в любую сторону. При закрытии створки сходятся вместе. Открытие может происходит с использованием одной или обеих частей. Легкость управления позволяет сдвигать элементы даже ребенку.</li>
            <li>Подъемно-раздвижная модель. Рекомендуются для использования в широких открытых проемах. Принцип действия идентичен с открыванием шкафа-купе. То есть один элемент заходит за другой на основе направляющего рельса. Несмотря на большие размеры, модель отличается надежной герметичностью и непроницаемостью для влаги.</li>
            <li>Раздвижные пластиковые двери типа «гармошка». При этой конструкции обеспечивается полноценное открытие проема одним элементом. Данный вариант представлен соединением нескольких секций, складывающихся в небольшой блок. В сложенном состоянии занимает минимум пространства. Можно использовать только одну створку для прохода или сложить всю конструкцию сразу.</li>
         </ol>
         <p>Раздвижные двери отличаются ценой в Киеве на основе различных конструкций, размеров и дизайнерских решений.</p>
         <h2 class="title-post">Преимущества</h2>
         <p>Раздвижные двери купить в Киеве следует за легкость и функциональность. Основным достоинством раздвижных систем является то, что после открытия створки они не занимают пространство внутри помещений и отличаются высокой эстетичностью. При этом сочетают в себе отличную тепло- и шумоизоляцию. Ключевые преимущества:</p>
         <ol>
            <li>Отсутствует мертвая зона, существующая при открытии других типов изделий.</li>
            <li>Раздвижные стеклянные двери с приемлемой ценой в Киеве дополнительно радуют простотой эксплуатации.</li>
            <li>Плотное прилегание створок обеспечивает сохранность тепла.</li>
            <li>Потребитель может заказать у нас различные размеры с использованием надежной фурнитуры.</li>
            <li>Длительный срок службы и устойчивость к внешним негативным факторам воздействия.</li>
            <li>Конструкционные особенности позволяют изделию идеально сочетаться с любым видом ремонта.</li>
         </ol>
         <p>К тому же раздвижные двери в Киев недорого можно заказать на сайте нашей компании.</p>
         <h2 class="title-post">Недостатки</h2>
         <p>Раздвижные двери в Киеве, как и любой товар имеют определенные недостатки. Хотя шум из внешних источников удачно нивелируется подобной моделью, все-таки распашной вариант лучше препятствует проникновению сильных звуковых колебаний. Это связано с особенностью конструкции и не сильно плотным прилеганием створки. Вторым минусом потребители традиционно считают стоимость, превышающую бюджетные изделия.</p>
         <h2 class="title-post">Особенности конструкции</h2>
         <p>Раздвижные металлопластиковые двери по конструкции сопоставимы с окнами такого же типа.</p>
         <p>Из чего состоит изделие:</p>
         <ol>
            <li>Профиль характеризуется шириной и массивностью. Предварительно он армируется для долговечности и надежности. Сварные швы усиливаются дополнительными вкладышами.</li>
            <li>Специальная фурнитура. Данный товар подвержен большим нагрузкам и интенсивному пользованию. Дополняется ручками, замками, ограничителями и доводчиками. Удерживают вес мощные петли.</li>
            <li>Необходим монтаж порога. Он существенно влияет на дизайн всего изделия. Изготавливается в виде алюминиевой вставки или пластикового аналога.</li>
            <li>Раздвижные стеклянные двери в Киеве реализуются с различным заполнением. Стеклопакет может быть обычным, с дизайнерским витражом или проведенным тонированием. Для глухого блока применяют сэндвич-панель. Можно добавить декоративную раскладку.</li>
         </ol>
         <p>Любая модель будет сочетать в себе функциональность, надежность и современный дизайн.</p>
         <p>Как выбрать раздвижные металлопластиковые двери</p>
         <p>Наша компания WindowsGroup предлагает раздвижные двери по доступной цене в Украине от производителя.</p>
         <p>При выборе обязательно стоит акцентировать внимание на ключевых факторах:</p>
         <ol>
            <li>Место монтажа. Оно должно подходить под выбранную модель и обладать требуемой площадью для установки подобного изделия.</li>
            <li>Толщина профиля. Необходимо учитывать степень армирования в зависимости от размеров системы, нагрузки и интенсивности пользования.</li>
            <li>Качество фурнитуры. Экономия на этом параметре приведет к быстрому выходу дверей из эксплуатации. Следует приобретать надежную фурнитуру от проверенных производителей.</li>
         </ol>
         <p>Если потребитель желает раздвижные двери купить по оптимальной стоимости и с идеальным качеством, то ему прямая дорога в компанию WindowsGroup.</p>
      </div>
   </section>
