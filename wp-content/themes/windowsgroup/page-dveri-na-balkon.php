<?php
/* Template Name: PageDveriNaBalcon */
get_header();
?>
...
<div class="main page-windows page-type-windows">
   ...
   <section class="section-table-price">
      <div class="container">
         <div class="top-post-page">
             <h1 class="title-post">Двери на балкон металлопластиковые - Киев</h1>
             <p class="text-custom-page">Мы предлагаем огромный выбор балконных металлопластиковых дверей напрямую от известных производителей.
                 В наших каталогах представлены популярные универсальные дверные модели по выгодным ценам.
                 Купить пластиковые двери из металлопластика – это значит приобрести надежную и долговечную конструкцию.</p>
         </div>

         <?php 
            
            global $wpdb;
            $result_params = $wpdb->get_results("SELECT * FROM `{$wpdb -> prefix}window_prices` WHERE `page_name` = 'dvery_na_balcon' ");
         ...
         <div class="block-windows-type-1">
            ...
            <span class="small-title">Размер 600 х 2000 мм + 1100 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->s_gluhim_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->s_gluhim_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-open">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_gluhim_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-open" role="tabpanel" aria-labelledby="<?=$key_w?>-open">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/1-metalloplastikovyye-steklyannyye-dveri-na-balkon-s-glukhim-oknom.063d4237.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_gluhim_oknom->$key_w;
                              $j = 0;
         ...
         <div class="block-windows-type-2">
            ...
            <span class="small-title">Размер 600 х 2000 мм + 1100 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->s_otkritym_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->s_otkritym_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_otkritym_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>" role="tabpanel" aria-labelledby="<?=$key_w?>">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/2-metalloplastikovyye-steklyannyye-dveri-na-balkon-s-otkryvayushchimsya-oknom.2b37d538.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_otkritym_oknom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-3">
            ...
            <span class="small-title">Размер 600 х 2000 мм + 1100 х 2000 мм, Цена от <span class="min-price"><?=$price_obj->s_panoramnym_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->s_panoramnym_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-3">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_panoramnym_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-3" role="tabpanel" aria-labelledby="<?=$key_w?>-type-3">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/3-panoramnyye-metalloplastikovyye-steklyannyye-dveri-na-balkon-s-panoramnym-oknom.1113694d.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_panoramnym_oknom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-4">
            <h2 class="title-block-type-windows">Панорамные металлопластиковые двери на балкон с сэндвич панелью и глухим окном</h2>
            <span class="small-title">Размер 600 х 2000 мм + 1100 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->s_sendvich_gluhim_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->s_sendvich_gluhim_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-4">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_sendvich_gluhim_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-4" role="tabpanel" aria-labelledby="<?=$key_w?>-type-4">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/4-panoramnyye-metalloplastikovyye-dveri-na-balkon-s-sendvich-panelyu-i-glukhim-oknom.e55876f6.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_sendvich_gluhim_oknom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-5">
            <h2 class="title-block-type-windows">Панорамные металлопластиковые двери на балкон с сэндвич панелью и открывающимся окном</h2>
            <span class="small-title">Размер 600 х 2000 мм + 1100 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->s_sendvich_otkritym_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->s_sendvich_otkritym_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-5">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_sendvich_otkritym_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-5" role="tabpanel" aria-labelledby="<?=$key_w?>-type-5">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/5-panoramnyye-metalloplastikovyye-dveri-na-balkon-s-sendvich-panelyu-i-otkryvayushchimsya-oknom.e4e5322b.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_sendvich_otkritym_oknom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-6">
            <h2 class="title-block-type-windows">Штульповые металлопластиковые двери на балкон с глухим окном</h2>
            <span class="small-title">Размер 1200 х 2100 мм, окно 750 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->shtulpoviye_s_gluhim_oknom->min?></span> грн до <span class="max-price"><?=$price_obj->shtulpoviye_s_gluhim_oknom->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-6">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->shtulpoviye_s_gluhim_oknom as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-6" role="tabpanel" aria-labelledby="<?=$key_w?>-type-6">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/6-shtulpovyye-metalloplastikovyye-dveri-na-balkon-s-glukhim-oknom.57147bca.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->shtulpoviye_s_gluhim_oknom->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-7">
            <h2 class="title-block-type-windows">Металлопластиковые двери на балкон с открывающимся окном и фрамугой</h2>
            <span class="small-title">Размер двери 800 х 2100, окно 1100 х 1350, фрамуга 1900 х 500 мм, Цена от <span class="min-price"><?=$price_obj->s_otkritym_oknom_framogoy->min?></span> грн до <span class="max-price"><?=$price_obj->s_otkritym_oknom_framogoy->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-7">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_otkritym_oknom_framogoy as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-7" role="tabpanel" aria-labelledby="<?=$key_w?>-type-7">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/7-metalloplastikovyye-dveri-na-balkon-s-otkryvayushchimsya-oknom-i-framugoy.3ed015f7.png" class="vertical-doors-3 img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_otkritym_oknom_framogoy->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-8">
            <h2 class="title-block-type-windows">Металлопластиковые двери на балкон с открывающимся окном и двойной фрамугой</h2>
            <span class="small-title">Размер двери 800 х 2100, окна 750 х 1350, фрамуги 1550 х 500 мм, Цена от <span class="min-price"><?=$price_obj->s_otkritym_oknom_2_framogoy->min?></span> грн до <span class="max-price"><?=$price_obj->s_otkritym_oknom_2_framogoy->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-8">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->s_otkritym_oknom_2_framogoy as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-8" role="tabpanel" aria-labelledby="<?=$key_w?>-type-8">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/8-metalloplastikovyye-dveri-na-balkon-s-otkryvayushchimsya-oknom-i-dvoynoy-framugoy.8c2420c1.png" class="vertical-doors-3 img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->s_otkritym_oknom_2_framogoy->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-9">
            <h2 class="title-block-type-windows">Одностворчатая металлопластиковая стеклянная дверь на балкон</h2>
            <span class="small-title">Размер 800 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->odnastvorchataya->min?></span> грн до <span class="max-price"><?=$price_obj->odnastvorchataya->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-9">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->odnastvorchataya as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-9" role="tabpanel" aria-labelledby="<?=$key_w?>-type-9">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/9-odnostvorchataya-metalloplastikovaya-steklyannaya-dver-na-balkon.b3ba2de2.png" class="vertical-doors img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->odnastvorchataya->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-10">
            <h2 class="title-block-type-windows">Одностворчатая металлопластиковая стеклянная дверь на балкон с сэндвич панелью</h2>
            <span class="small-title">Размер 800 х 2100 мм, Цена от <span class="min-price"><?=$price_obj->odnastvorchataya_s_sendvich->min?></span> грн до <span class="max-price"><?=$price_obj->odnastvorchataya_s_sendvich->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-10">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->odnastvorchataya_s_sendvich as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-10" role="tabpanel" aria-labelledby="<?=$key_w?>-type-10">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/10-odnostvorchataya-metalloplastikovaya-dver-na-balkon-s-sendvich-panelyu.29198d45.png" class="vertical-doors img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->odnastvorchataya_s_sendvich->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
         ...
         <div class="block-windows-type-11">
            <h2 class="title-block-type-windows">Одностворчатая металлопластиковая стеклянная дверь на балкон с фрамугой</h2>
            <span class="small-title">Размер двери 800 х 2100 + окно глухое 800 х 500 мм, Цена от <span class="min-price"><?=$price_obj->odnastvorchataya_s_framogoy->min?></span> грн до <span class="max-price"><?=$price_obj->odnastvorchataya_s_framogoy->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-type-11">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->odnastvorchataya_s_framogoy as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-type-11" role="tabpanel" aria-labelledby="<?=$key_w?>-type-11">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/11-odnostvorchataya-metalloplastikovaya-dver-na-balkon-s-framugoy.0e1b5c95.png" class="vertical-doors img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->odnastvorchataya_s_framogoy->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
   ...
   <section class="section-post section-post-windows">
      <div class="container">
         <h2 class="title-section">Преимущества металлопластиковых дверей на балкон</h2>
         <p>Все чаще потребители выбирают металлопластиковые дверные конструкции. Такой выбор обуславливается особыми преимуществами типа изделий из ПВХ:</p>
         <ol>
            <li>Устойчивы к деформации и коррозии.</li>
            <li>Просты в уходе.</li>
            <li>Не требуют покраски.</li>
            <li>Создают барьер против любого шума.</li>
            <li>Усиливают сохранение тепла внутри помещения.</li>
            <li>Регулируют проветривание комнаты.</li>
         </ol>
         <p>Выбор различных дверных моделей открывает возможность для дизайнерской фантазии в оформлении балкона. В ассортименте нашего сайта представлены пластиковые балконные двери по доступным ценам. Разные формы, размеры и цветовые решения помогут сделать вашу лоджию комфортной и уютной.</p>
         <h2 class="title-post">Разновидности</h2>
         <p>Существует несколько типов пластиковых дверей на балкон:</p>
         <ol>
            <li>Поворотные или распашные (раскрываются на или от себя, аналогично межкомнатной конструкции).</li>
            <li>Поворотно-откидные (открываются по тому же принципу, что и поворотные, дополнительно фиксируются).</li>
            <li>Штульповые (имеют две створки, разделенные посередине импостом).</li>
            <li>Двери-гармошки (отличаются наличием нескольких подвижных створок, перемещаемых по принципу складывания книги).</li>
            <li>Раздвижные (раскрываются наподобие дверей-купе: раздвигаются вдоль стены или складываются в нишу).</li>
         </ol>
         <p>Установка раздвижных дверей на балкон позволит сэкономить пространство в комнате. По типу раздвижные модели бывают классические и откидные. Второй тип можно слегка откинуть или максимально открыть для проветривания. Стеклянные раздвижные створки обеспечивают отличное попадание солнца в помещение. Они служат дополнительным источником света. Раздвижные двери абсолютно безопасные: ими невозможно удариться при открывании.</p>
         <h2 class="title-post">Из чего состоит дверь, фурнитура</h2>
         <p>Пластиковые балконные двери состоят из следующих элементов:</p>
         <ul>
            <li>· стеклопакета;</li>
            <li>· фурнитуры;</li>
            <li>· многокамерного армированного ПВХ-профиля;</li>
            <li>· порога;</li>
            <li>· декоративных элементов.</li>
         </ul>
         <p>Балконная дверь представляет собой высокое окно, через которое мы проходим на лоджию. Цена на конструкцию сравнительно недорогая за счет изготовления из оконных створок. Максимальная высота створки составляет 2350 мм, при предельных показателях – не сработает поворотно-откидная фурнитура. В более высоких дверях в верхней части размещается открывающееся или глухое окошко (фрамуга). Кроме того внизу вместо стеклопакета может размещаться непрозрачная сендвич-панель.</p>
         <p>В качестве фурнитуры, как и в окнах, используется поворотно-откидной механизм. С наружной стороны балконный комплект оснащается ручкой ухватом и защелкой. Для защиты от случайного закрытия конструкции может использоваться двухсторонняя ручка В редких случаях имеется ручка с ключиком, служащая блокировкой от открывания ребенком.</p>
         <p>Устойчивость и надежность конструкции обеспечивается армированием. Металлическая основа в пластиковом дверном профиле повышает сопротивление ветровым нагрузкам и предотвращает деформацию балконной конструкции.</p>
         <p>Ширина двери на балкон варьируется в пределах 1000 мм. При большем значении ширины изготавливаются двухстворчатые модели. В данном дверном типе используется порог в виде оконной рамы. Это предотвращает продувание и промерзание помещения.</p>
         <p>В качестве крепления используются оконные петли, эксплуатация которых рассчитана на 40 000 открываний и закрываний. Это указывает на то, что данный дверной вид не рекомендуется устанавливать при входе в административные здания.</p>
         <h2 class="title-post">Как правильно выбрать</h2>
         <p>Подбирая дверь, необходимо учесть ряд нюансов:</p>
         <ul>
            <li>· особенности конструкции (одна или две створки), габариты;</li>
            <li>· тип открывания-закрывания;</li>
            <li>· вид армирующего профиля;</li>
            <li>· вспомогательные элементы (москитную сетку, порог, шпросы);</li>
            <li>· необходимость тонировки или ламинирования (значительно влияет на стоимость изделия).</li>
         </ul>
         <p>Для маленьких помещений рекомендуется выбирать цельные конструкции. В больший проем предпочтительнее устанавливать раздвижные модели. Данный тип идеально подходит для комнат с высоким потолком из-за того, что в фурнитуре открывания присутствуют ограничения.</p>
         <p>Перед тем, как купить балконные металлопластиковые двери по лучшим ценам в Украине, необходимо правильно выбрать поставщика. Мы предлагаем заказать конструкции для балкона у нас, так как наша компания готова предложить наиболее выгодные цены.</p>
         <p>Узнать цены в Киеве на балконную дверь с окном можно на сайте в прайсе или позвонить по указанным телефонам.</p>
      </div>
   </section>
      ...
      <div class="container">
         ...
         <ul class="list-other-type-windows">
            ...
            <li class="item-other-type-windows">
               ...
               <a href="<?=get_bloginfo('home')?>/plastikovye-dveri/balkonnye-bloki/" class="title-footer">Балконные Блоки</a>
            </li>
         </ul>
      </div>
   </section>
