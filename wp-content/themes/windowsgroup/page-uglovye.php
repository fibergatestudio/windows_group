...
<div class="main page-windows page-type-windows">
   ...
   <section class="section-table-price">
      <div class="container">
         <div class="top-post-page">
            <h1 class="title-post">Угловые ПВХ (пластиковые) окна в Киеве – цены</h1>
            <p class="text-custom-page">Угловое окно – тип остекления в углу постройки без несущей капитальной опоры.
                Нестандартная оконная конструкция сделает фасад здания эффектным.
                Компания «Виндовс Груп» предлагает купить угловое окно в Киеве и области по лучшим ценам.</p>
         </div>

         <?php 
            
            global $wpdb;
            $result_params = $wpdb->get_results("SELECT * FROM `{$wpdb -> prefix}window_prices` WHERE `page_name` = 'okna_ugloviye' ");
            $price_obj = json_decode($result_params[0]->data);
         
         ?>
         
         <div class="block-windows-type-1">
            <h2 class="title-block-type-windows">Угловые Окна</h2>
            <span class="small-title">Размер 2250 х 2600 мм, Цена от <span class="min-price"><?=$price_obj->ugloviye->min?></span> грн до <span class="max-price"><?=$price_obj->ugloviye->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent-open">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->ugloviye as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>-open" role="tabpanel" aria-labelledby="<?=$key_w?>-open">
                  <div class="table-tab">
                     <div class="block-img mt-0"><img src="<?php bloginfo('template_url') ?>/images/corner-windows-1.3f1db37c.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->ugloviye->$key_w;
                              $j = 0;
         ...
         <div class="block-windows-type-2">
            <h2 class="title-block-type-windows">Угловые Окна</h2>
            <span class="small-title">Размер 2000 х 1350 мм, Цена от <span class="min-price"><?=$price_obj->ugloviye_2->min?></span> грн до <span class="max-price"><?=$price_obj->ugloviye_2->max?></span> грн</span>
            ...
            <div class="tab-content" id="myTabContent">
               ...
               <?php 
               $k = 0;
               foreach ($price_obj->ugloviye_2 as $key_w => $value_w) {
                  if (is_object($value_w)) { ?>

               <div class="tab-pane fade show <?=($k===0)?'active':''?>" id="<?=$key_w?>" role="tabpanel" aria-labelledby="<?=$key_w?>">
                  <div class="table-tab">
                     <div class="block-img"><img src="<?php bloginfo('template_url') ?>/images/corner-windows-2.fb9b0ce1.png" class="img-type-windows img-fluid" alt="photo"></div>
                     <table class="table-windows table-price-windows">
                        <thead>
                           <tr>
                              <td>Профиль</td>
                        ...
                        <tbody>
                           ...
                           <tr>
                              <?php
                              $this_obj = $price_obj->ugloviye_2->$key_w;
                              $j = 0;
                              foreach ($this_obj as $value) {
                                 $j++;
                              }                             
   ...
   <section class="section-post section-post-windows">
      <div class="container">
         <h2 class="title-section">Преимущества и недостатки угловых окон</h2>
         <p>Евроокна характеризуются рядом преимуществ:</p>
         <ol>
            <li>Проникновением максимального количества солнечного света в помещение.</li>
            <li>Необычным дизайном фасада и стилем интерьера.</li>
            <li>Возможностью применения стандартных оконных конструкций, соединенных в блок. Данный вариант существенно снижает стоимость углового остекления.</li>
            <li>Снижением общего веса для разгрузки опорных элементов постройки.</li>
         </ol>
         <p>Также угловые окна в доме характеризуются некоторыми недостатками использования:</p>
         <ol>
            <li>Блок оконной системы не является несущим. Монтаж возможен в зданиях, рассчитанных на данный тип остекления. Возможность установки продумывается при проектировании строения.</li>
            <li>В многоквартирных постройках перед началом монтажа требуется получение разрешения на изменение фасада.</li>
            <li>Конструкции подразумевают грамотный монтаж.</li>
         </ol>
         <p>Наши специалисты готовы произвести профессиональную установку изделия.</p>
         <h2 class="title-post">Особенности конструкции</h2>
         <p>Оконные модели делятся в зависимости от типа конструкции. Бывают нескольких разновидностей:</p>
         <ul>
            <li>· из цельного стеклянного полотна;</li>
            <li>· с типовой оконной рамой;</li>
            <li>· со стационарным профилем ПВХ.</li>
         </ul>
         <p>В евроокнах из цельного стекла применяется специальная технология сварки и прозрачного герметика. В качестве элементов крепления используется специальный угловой профиль.</p>
         <p>Модели с типовыми рамами стыкуются в углах обычным способом крепления без необходимых материалов.</p>
         <p>В моделях из традиционного профиля ПВХ в углу фиксируется рама со стеклопакетом или стеклянные вставки для панорамного остекления.</p>
         <p>Панорамные угловые окна значительно увеличат обзор из окна. Варианты расположения оконных створок и их количество рассчитывается в индивидуальном порядке.</p>
         <h2 class="title-post">Кому подойдут</h2>
         <p>Угловые светопрозрачные конструкции используются в элитных зданиях, оформленных в стиле минимализма. Также применяются при остеклении частных домов.</p>
         <p>Угловое окно в интерьере представлено в нескольких типах оформления:</p>
         <ol>
            <li>Конструкция в гостиной идеально подойдет для прямоугольной комнаты, с остеклением эркерных фронтальных элементов.</li>
            <li>Светопрозрачные оконные модели в спальне уместны для тех, кто любит наблюдать из окна. Для эстетики и регулирования дневного света подойдут непрозрачный тюль, жалюзи, рулонные шторы.</li>
            <li>Угловое окно на кухне увеличит количество солнечного освещения. Правильное расположение евроокна создаст уютную рабочую зону.</li>
         </ol>
         <p>Для объединения угловой конструкции рекомендуется использовать небольшие ламбрекены.</p>
         <h2 class="title-post">Как выбрать</h2>
         <p>При покупке следует учитывать нюансы:</p>
         <ol>
            <li>Материал изготовления евроокна. Конструкция должна быть прочной, иметь качественную тепло- и звукоизоляцию. Идеальный вариант – купить изделие из ПВХ профиля с армирующим металлом.</li>
            <li>Качество фурнитуры и уплотнителя влияет на устойчивость и надежность конструкции.</li>
            <li>Использование энергосберегающего стеклопакета сохранит тепло в помещении. Для защиты от солнца предпочтительнее выбирать стекла с i-напылением.</li>
         </ol>
         <p>Дополнительную защиту от взлома помещения обеспечит установка роллета или решетки.</p>
         <h2 class="title-post">Цены на угловые окна</h2>
         <p>Цена зависит от материала, габаритов, необходимого количества створок и функциональных возможностей конструкции. Заказать евроокно ПВХ можно на нашем сайте или позвонив по указанному телефону. Ориентировочные цены на оконные изделия представлены в прайс-листе нашей компании «Виндовс Груп».</p>
      </div>
   </section>
      ...
      <div class="container">
         ...
         <ul class="list-other-type-windows">
            <li class="item-other-type-windows">
               <div class="block-img-other"><img src="<?php bloginfo('template_url') ?>/images/cycle.a7c36460.webp" class="p-2 img-fluid" alt="photo"></div>
